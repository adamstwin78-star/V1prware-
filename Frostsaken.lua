-- Frostsaken | by mitsuki
-- Rayfield GUI | ESP color pickers
...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: VISUAL (ESP) — original v4 logic, custom colors preserved
------------------------------------------------------------------------
------------------------------------------------------------------------
...
tabVisual:CreateParagraph({ Title = "How it works", Content = "Attaches a Highlight and a BillboardGui label to each tracked object. Fully event-driven — ChildAdded/ChildRemoved connections update ESP instantly when players spawn or objects appear, with zero polling loops." })
...
tabVisual:CreateColorPicker({
    Name = "Item Color", Color = espColors.item, Flag = "espColItem",
    Callback = function(col)
...
tabVisual:CreateParagraph({ Title = "How it works", Content = "Tracks c00lkidd's pizza bots (orange), 1x1x1x1 zombies (green), and John Doe's shadow puddles (black disc with red glow). Each uses AncestryChanged to self-clean when removed." })
...
local function addPuddleHighlight(part, color, tag, label)
    ...
    task.defer(function()
        ...
        holder.CanCollide=false; holder.CanTouch=false; holder.Anchored=true
        holder.CFrame=CFrame.new(part.Position+Vector3.new(0,0.05,0)); holder.Parent=svc.WS
...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: NOLI — Coming Soon
-- Backend logic (heli + nova) is preserved below for future use
------------------------------------------------------------------------
------------------------------------------------------------------------
...
tabNoli:CreateParagraph({
    Title   = "🔒 Coming Soon",
    Content = "Noli features (Heli-Tech Macro, Nova Silent Aim) are currently locked and will be available in a future update.",
})

------------------------------------------------------------------------
-- BACKEND: Heli-Tech logic (disabled — UI coming soon)
------------------------------------------------------------------------
local heli = {
    enabled  = false,
...
-- Heli RenderStepped — only fires when heli.enabled (always false until unlocked)
svc.Run.RenderStepped:Connect(function()
...
------------------------------------------------------------------------
-- BACKEND: Nova Silent Aim logic (disabled — UI coming soon)
------------------------------------------------------------------------
local nova = {
    enabled          = false,
